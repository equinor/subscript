...
Documentation: https://wiki.equinor.com/wiki/index.php/ResScript:Python:Scripts:presentvalue
...
def main():
    ...
    parser.add_argument(
        "--oilvector",
        type=str,
        help="Eclipse vector to read cumulative oil production from. Use None to ignore.",
        default="FOPT",
    )
    parser.add_argument(
        "--gasvector",
        type=str,
        help="Eclipse vector to read cumulative gas production from. Use None to ignore.",
        default="FGPT",
    )
    parser.add_argument(
        "--gasinjvector",
        type=str,
        help="Eclipse vector to read cumulative gas injection from. Use None to ignore.",
        default="FGIT",
    )
    parser.add_argument(
        "--cutoffyear",
    ...
    parser.add_argument(
        "--econtable",
        type=str,
        help="Comma separated table with years as rows, and column names specifying economical parameters. Supported column names: oilprice (USD/bbl), gasprice (NOK/sm3), usdtonok, costs (MNOK)",
    )
    parser.add_argument(
        "--basedatafiles",
        nargs="+",
        default=[],
        help="Input Eclipse DATA files to be used as base cases to calculate delta production profiles",
    )
    parser.add_argument(
        "--bepcalcmethod",
        type=int,
        default=0,
        help="Select a method to be used to calculate the break-even price. (0) EPA Q4 2015 External Assumptions; there is a link between gas and oil prices and the resulting break-even price is in USD/boe; (1) Fixed gas prices, the resulting break-even price is in USD/bbl. This can be used to evaluate gas injection/deferral projects.",
    )
    ...
    if not (
        ...
        sys.exit(
            "Supply either no base cases, a single base case or exactly as many base cases as datafiles. Script stopped."
        )
    ...
    for eclcase in args.datafiles:
        ...
        first_date = (
            summ.data_start.date()
        )  # If restart information can't be loaded, we must use this instead of summ.start_time.
