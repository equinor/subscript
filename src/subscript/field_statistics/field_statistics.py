...
def get_values_in_ertbox(
    ...
    if geogrid_subgrids:
        # Multi-zone grid
        if zone_name not in geogrid_subgrids:
            raise ValueError(f"Missing {zone_name} in geogrid subgrid")
...
def set_values_in_geogrid(
    ...
    if geogrid_subgrids:
        # Multi-zone grid
        if zone_name not in geogrid_subgrids:
            raise ValueError(f"Missing {zone_name} in geogrid subgrids")
    ...
    if (
        geogrid_dimensions[0] != ertbox_size[0]
        or geogrid_dimensions[1] != ertbox_size[1]
    ):
        raise ValueError(
            "Geogrid dimension "
            f"({geogrid_dimensions[0]}, {geogrid_dimensions[0]}) "
            f"for (nx, ny) must be the same as for the ertbox grid"
        )
    if ertbox_size[2] < nz_zone:
        raise ValueError(
            f" Ertbox grid size {ertbox_size[2]} < {nz_zone}. "
            "Must be at least as large as number of layers for the zone."
        )
...
def ertbox_to_geogrid_statistics(
    ...
    if (param_name is None) and (facies_name is None):
        raise ValueError(
            "Facies name and parameter name cannot both be undefined "
            "in ertbox_to_geogrid_statistics"
        )
...
def get_common_field_specifications(
    ...
    if ert_config_path:
        ...
        ertbox_size_dict = get_ertbox_size_per_zone(
            ertbox_path, zone_names, ertbox_per_zone_dict, ertbox_default
        )
    else:
        ertbox_size_dict = {}
...
def calc_stats(
    ...
    (
        ...
        input_dict, use_facies_per_zone=True, facies_per_zone=facies_per_zone
    )
